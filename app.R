...
kickstarter<-crowdfdata[-c(1:4)]
uvals<-sapply(kickstarter,function(x){length(unique(x))})
kickstarter<-map_if(kickstarter,uvals<4,as.factor) %>%
    as.data.frame()
...
ui<-(pageWithSidebar(
    ...
    (
        ...
        selectInput("dataset","Data:",
                    choices =list(iris = "iris", kickstarter = "kickstarter",
